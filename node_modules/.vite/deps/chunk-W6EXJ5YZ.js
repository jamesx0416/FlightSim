...
// node_modules/3d-tiles-renderer/build/constants-Cj07Qhs1.js
var z = class {
  ...
  add(t, a) {
    ...
    return o.push(t), s.add(t), e.set(t, Date.now()), i.set(t, a), true;
  }
  has(t) {
  ...
  remove(t) {
    ...
    if (e.has(t)) {
      ...
      const d2 = s.indexOf(t);
      return s.splice(d2, 1), a.delete(t), e.delete(t), i.delete(t), c.delete(t), true;
    }
    return false;
  }
  ...
  unloadUnusedContent() {
    const {
      ...
      minBytesSize: h,
      maxBytesSize: y
    } = this, b2 = s.length - i.size, B2 = s.length - c.size, S2 = Math.max(Math.min(s.length - a, b2), 0), k = this.cachedBytes - h, M2 = this.unloadPriorityCallback || this.defaultPriorityCallback;
    let f = false;
    const P2 = S2 > 0 && b2 > 0 || B2 && s.length > e;
    if (b2 && this.cachedBytes > h || B2 && this.cachedBytes > y || P2) {
      s.sort((n, r2) => {
        const U2 = i.has(n), L = i.has(r2);
        if (U2 === L) {
          const x2 = c.has(n), v = c.has(r2);
          return x2 === v ? -M2(n, r2) : x2 ? 1 : -1;
        } else
          return U2 ? 1 : -1;
      });
      const A = Math.max(a * t, S2 * t), p = Math.ceil(Math.min(A, b2, S2)), E = Math.max(t * k, t * h), w = Math.min(E, k);
      let l = 0, m = 0;
      for (; this.cachedBytes - m > y || s.length - l > e; ) {
        const n = s[l], r2 = u.get(n) || 0;
        if (i.has(n) && c.has(n) || this.cachedBytes - m - r2 < y && s.length - l <= e)
          break;
        m += r2, l++;
      }
      for (; m < w || l < p; ) {
        const n = s[l], r2 = u.get(n) || 0;
        if (i.has(n) || this.cachedBytes - m - r2 < h && l >= p)
          break;
        m += r2, l++;
      }
      s.splice(0, l).forEach((n) => {
        this.cachedBytes -= u.get(n) || 0, d2.get(n)(n), u.delete(n), o.delete(n), d2.delete(n), c.delete(n), i.delete(n);
      }), f = l < S2 || m < k && l < b2, f = f && l > 0;
    }
...
var G = class {
  ...
  tryRunJobs() {
    ...
    for (; e > this.currJobs && t.length > 0 && s < e; ) {
      this.currJobs++, s++;
      const i = t.pop(), { callback: c, resolve: d2, reject: u } = a.get(i);
      a.delete(i);
      let h;
      try {
        h = c(i);
      ...
      h instanceof Promise ? h.then(d2).catch(u).finally(o) : (d2(h), o());
    }
  }
  scheduleJobRun() {
...
var F = 4;
var N = 6378137;
var j = 1 / 298.257223563;
var Q = 6356752314245179e-9;
...
// node_modules/3d-tiles-renderer/build/MemoryUtils-BD0AS93P.js
var b = new Spherical();
var et = new Vector3();
var _t = {};
function xt(c) {
...
function Rt(c, t, i = {}) {
  return b.theta = t, b.phi = J2(c), et.setFromSpherical(b), b.setFromVector3(et), i.lat = gt(b.phi), i.lon = b.theta, i;
}
...
function bt(c, t, i = false) {
  const o = Rt(c, t, _t);
  let e, s;
  return i ? (e = `${(MathUtils.RAD2DEG * o.lat).toFixed(4)}°`, s = `${(MathUtils.RAD2DEG * o.lon).toFixed(4)}°`) : (e = st(MathUtils.RAD2DEG * o.lat, "N", "S"), s = st(MathUtils.RAD2DEG * o.lon, "E", "W")), `${e} ${s}`;
}
var Ut = Object.freeze(Object.defineProperty({
  __proto__: null,
  latitudeToSphericalPhi: J2,
  sphericalPhiToLatitude: gt,
  swapToGeoFrame: xt,
  swapToThreeFrame: Ct,
  toLatLonString: bt
}, Symbol.toStringTag, { value: "Module" }));
var nt = new Spherical();
var z2 = new Vector3();
var x = new Vector3();
var Y = new Vector3();
var P = new Matrix4();
...
var Mt = class {
  ...
  getEastNorthUpFrame(t, i, o, e) {
    return o.isMatrix4 && (e = o, o = 0, console.warn('Ellipsoid: The signature for "getEastNorthUpFrame" has changed.')), this.getEastNorthUpAxes(t, i, at, lt, ct), this.getCartographicToPosition(t, i, o, _2), e.makeBasis(at, lt, ct).setPosition(_2);
  }
  ...
  getCartographicFromObjectFrame(t, i, o = U) {
    return o === ht ? (g.set(-Math.PI / 2, 0, 0, "XYZ"), S.makeRotationFromEuler(g).premultiply(t)) : o === U ? (g.set(-Math.PI / 2, 0, Math.PI, "XYZ"), S.makeRotationFromEuler(g).premultiply(t)) : S.copy(t), _2.setFromMatrixPosition(S), this.getPositionToCartographic(_2, i), this.getEastNorthUpFrame(i.lat, i.lon, 0, P).invert(), S.premultiply(P), g.setFromRotationMatrix(S, "ZXY"), i.azimuth = -g.z, i.elevation = g.x, i.roll = g.y, i;
  }
  getEastNorthUpAxes(t, i, o, e, s, n = _2) {
  ...
  getAzElRollFromRotationMatrix(t, i, o, e, s = q) {
    return console.warn('Ellipsoid: "getAzElRollFromRotationMatrix" is deprecated. Use "getCartographicFromObjectFrame", instead.'), this.getCartographicToPosition(t, i, 0, _2), rt.copy(o).setPosition(_2), this.getCartographicFromObjectFrame(rt, e, s), delete e.height, delete e.lat, delete e.lon, e;
  }
  ...
  getCartographicToPosition(t, i, o, e) {
    this.getCartographicToNormal(t, i, z2);
    const s = this.radius;
    x.copy(z2), x.x *= s.x ** 2, x.y *= s.y ** 2, x.z *= s.z ** 2;
    const n = Math.sqrt(z2.dot(x));
    return x.divideScalar(n), e.copy(x).addScaledVector(z2, o);
  }
  getPositionToCartographic(t, i) {
    this.getPositionToSurfacePoint(t, x), this.getPositionToNormal(t, z2);
    const o = Y.subVectors(t, x);
    return i.lon = Math.atan2(z2.y, z2.x), i.lat = Math.asin(z2.z), i.height = Math.sign(o.dot(t)) * o.length(), i;
  }
  getCartographicToNormal(t, i, o) {
    return nt.set(1, J2(t), i), o.setFromSpherical(nt).normalize(), xt(o), o;
  }
  ...
  getPositionToSurfacePoint(t, i) {
    ...
    let m = (1 - w) * t.length() / (0.5 * E.length()), y = 0, k, K, v, A, N2, L, V, X, Q2, tt, it;
    do {
      m -= y, v = 1 / (1 + m * e), A = 1 / (1 + m * s), N2 = 1 / (1 + m * n), L = v * v, V = A * A, X = N2 * N2, Q2 = L * v, tt = V * A, it = X * N2, k = a * L + l * V + p * X - 1, K = a * Q2 * e + l * tt * s + p * it * n;
      const Pt = -2 * K;
      y = k / Pt;
    } while (Math.abs(k) > vt);
    return i.set(
...
var jt = class {
  ...
  intersectsRay(t) {
    return G2.copy(t).applyMatrix4(this.inverseTransform), G2.intersectsBox(this.box);
  }
  // Sets "target" equal to the intersection point.
  // Returns "null" if no intersection found.
  intersectRay(t, i) {
    return G2.copy(t).applyMatrix4(this.inverseTransform), G2.intersectBox(this.box, i) ? (i.applyMatrix4(this.transform), i) : null;
  }
  update() {
  ...
  updatePlanes() {
    j2.copy(this.box.min).applyMatrix4(this.transform), $.copy(this.box.max).applyMatrix4(this.transform), M.set(0, 0, 1).transformDirection(this.transform), this.planes[0].setFromNormalAndCoplanarPoint(M, j2), this.planes[1].setFromNormalAndCoplanarPoint(M, $).negate(), M.set(0, 1, 0).transformDirection(this.transform), this.planes[2].setFromNormalAndCoplanarPoint(M, j2), this.planes[3].setFromNormalAndCoplanarPoint(M, $).negate(), M.set(1, 0, 0).transformDirection(this.transform), this.planes[4].setFromNormalAndCoplanarPoint(M, j2), this.planes[5].setFromNormalAndCoplanarPoint(M, $).negate();
  }
  intersectsSphere(t) {
...
var $t = class extends Mt {
  constructor(t = 1, i = 1, o = 1, e = -H, s = H, n = 0, a = 2 * I2, l = 0, p = 0) {
  ...
  getBoundingBox(t, i) {
    ...
    w ? E = o : f ? E = e : E = 0;
    const { min: m, max: y } = t;
    m.setScalar(1 / 0), y.setScalar(-1 / 0), n - s <= I2 ? (this.getCartographicToNormal(p, h, T), C2.set(0, 0, 1), B.crossVectors(C2, T).normalize(), C2.crossVectors(T, B).normalize(), i.makeBasis(B, C2, T), d.copy(i).invert(), this.getCartographicToPosition(E, s, l, r).applyMatrix4(d), y.x = Math.abs(r.x), m.x = -y.x, this.getCartographicToPosition(e, s, l, r).applyMatrix4(d), y.y = r.y, this.getCartographicToPosition(e, h, l, r).applyMatrix4(d), y.y = Math.max(r.y, y.y), this.getCartographicToPosition(o, s, l, r).applyMatrix4(d), m.y = r.y, this.getCartographicToPosition(o, h, l, r).applyMatrix4(d), m.y = Math.min(r.y, m.y), this.getCartographicToPosition(p, h, l, r).applyMatrix4(d), y.z = r.z, this.getCartographicToPosition(o, s, a, r).applyMatrix4(d), m.z = r.z, this.getCartographicToPosition(e, s, a, r).applyMatrix4(d), m.z = Math.min(r.z, m.z)) : (this.getCartographicToPosition(E, h, l, T), T.z = 0, T.length() < 1e-10 ? T.set(1, 0, 0) : T.normalize(), C2.set(0, 0, 1), B.crossVectors(T, C2).normalize(), i.makeBasis(B, C2, T), d.copy(i).invert(), this.getCartographicToPosition(E, h + H, l, r).applyMatrix4(d), y.x = Math.abs(r.x), m.x = -y.x, this.getCartographicToPosition(e, 0, f ? a : l, r).applyMatrix4(d), y.y = r.y, this.getCartographicToPosition(o, 0, w ? a : l, r).applyMatrix4(d), m.y = r.y, this.getCartographicToPosition(E, h, l, r).applyMatrix4(d), y.z = r.z, this.getCartographicToPosition(E, n, l, r).applyMatrix4(d), m.z = r.z), t.getCenter(r), t.min.sub(r).multiplyScalar(1 + W), t.max.sub(r).multiplyScalar(1 + W), r.applyMatrix4(i), i.setPosition(r);
  }
  getBoundingSphere(t) {
    ...
    let f;
    h ? f = i : w ? f = o : f = 0, this.getCartographicToPosition(f, e, a, r), F2(r, t), this.getCartographicToPosition(o, e, a, r), F2(r, t), this.getCartographicToPosition(o, p, a, r), F2(r, t), this.getCartographicToPosition(i, e, a, r), F2(r, t), this.getCartographicToPosition(i, p, a, r), F2(r, t), this.getCartographicToPosition(l, p, a, r), F2(r, t), this.getCartographicToPosition(i, e, n, r), F2(r, t), s - e > I2 && (this.getCartographicToPosition(f, p + I2, a, r), F2(r, t)), t.radius = Math.sqrt(t.radius) * (1 + W);
  }
};
function ft(c) {
  if (!c)
...
//# sourceMappingURL=chunk-W6EXJ5YZ.js.map
