...
2026-06-05: WebGPU blend-gbuffer render-target depth fallback must resolve the receiver depth immediately after the base pass while all blend-gbuffer materials are still hidden. Resolving after restored color blend-gbuffer materials lets the overlay/decal layer contribute to the sampled mask, which can erase the stabilizer-root/base attachment instead of only removing the floating overlay.
