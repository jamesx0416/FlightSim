...
test('publishes key-event control and electrical state without waiting for a tick', () => {
  ...
  test('spoiler object positions remain animation-local', () => {
    ...
    // Package Update code owns these O: mirrors; only K/B/A/L contracts control spoilers.
...
test('routes MSFS key events into canonical cold-start system commands', () => {
  ...
  expect(host.simulatorEngine.state.readBoolean(PropulsionStateKeys.engineCombustion(1))).toBe(true)
  const n1Percent = host.readVariable('A:TURB ENG N1:1', 'percent')
  expect(n1Percent > 0).toBe(true)
  expect(host.readVariable('A:TURB ENG CORRECTED N1:1', 'percent')).toBe(n1Percent)
  expect(host.readVariable('A:TURB ENG N2:1', 'percent')).toBe(n1Percent)
  expect(host.readVariable('A:GENERAL ENG RPM:1')).toBe(n1Percent * 100)
  expect(host.readVariable('A:GENERAL ENG COMBUSTION:1', 'Bool')).toBe(1)
  expect(host.readVariable('A:GENERAL ENG STARTER:1', 'Bool')).toBe(1)
