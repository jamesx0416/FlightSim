...
export class AircraftRuntime {
  ...
  bindAnimations(clips: readonly AnimationClip[]): void {
  ...
  update(
    ...
    this.mixer.update(0)
    mixerMs = finishPhase()
    this.captureWingFlexAuthoredPoses()
  ...
  private captureWingFlexAuthoredPoses(): void {
    for (const binding of this.wingFlexBindings) {
      captureWingFlexAttachmentPoses(binding.attachments, binding.root)
      captureWingFlexSurfaceCpuPoses(binding.surfaceCpuRepairs, binding.root)
    }
  }
...
export class SharedMsfsRuntimeHost implements RuntimeHostServices {
 ...
 const normalizedValue = Number.isFinite(value) && value > 0 ? 1 : 0
  ...
  private resolveHeuristicValue(
    ...
    if (upperKey === 'A:WING FLEX PCT' || upperKey.startsWith('A:WING FLEX PCT:')) {
      const physicsState = this.simulatorEngine.state
      const hasPhysicsFlex = physicsState.getDefinition(AirPhysicsStateKeys.wingLeftFlexRatio()) != null
      const sideIndex = upperKey === 'A:WING FLEX PCT'
        ? 0
        : Number.parseInt(upperKey.split(':').at(-1) ?? '0', 10)
      if (hasPhysicsFlex) {
        const left = physicsState.readNumber(AirPhysicsStateKeys.wingLeftFlexRatio(), { fallback: 0 }) ?? 0
        const right = physicsState.readNumber(AirPhysicsStateKeys.wingRightFlexRatio(), { fallback: 0 }) ?? 0
        const flexRatio = sideIndex === 1 ? left : sideIndex === 2 ? right : (left + right) * 0.5
        return handled(convertPercentOver100Unit(flexRatio, unit))
      }
      const flexPct =
...
function createCanonicalSystemDefinitions(
  ...
  const propulsion: CanonicalPropulsionSystemConfig = {
    ...basePropulsion,
    engines: addMsfsPropulsionPhysicsMetadata(basePropulsion.engines ?? [], aircraft),
  }
  const airPhysics = createMsfsAirPhysicsSystemConfig(aircraft)
  ...
  return [
    ...
    ...(airPhysics == null
      ? []
      : [{ id: 'air-physics', kind: 'air-physics' as const, config: airPhysics }]),
  ]
}
...
interface RuntimeWingFlexAttachment {
  ...
  readonly authoredQuaternionInRoot: Quaternion
  readonly rotateWithWing: boolean
}
...
interface RuntimeWingFlexSurfaceCpuRepair {
...
interface RuntimeWingFlexBinding {
  ...
  readonly surfaceCpuRepairs: RuntimeWingFlexSurfaceCpuRepair[]
  readonly maxAngleRadians: number
}

type RuntimeSkinAttribute = ReturnType<SkinnedMesh['geometry']['getAttribute']>
...
function buildWingFlexBindings(
  ...
  for (const animation of nodeAnimations) {
    ...
    for (const nodeName of animation.nodes) {
      ...
      if (node == null || descriptor == null) continue
      const entry = { node, order: descriptor.index }
    ...
    smoothMainWingFlexSkinWeights(
      sceneRoot,
      [leftWing, rightWing].filter((side): side is RuntimeWingFlexSide => side != null)
    )
...
function getRuntimeWingFlexSourceSkinning(
  mesh: SkinnedMesh
): RuntimeWingFlexSourceSkinning | null {
  const existing = runtimeWingFlexSourceSkinning.get(mesh.geometry)
  if (existing != null) return existing
  const skinIndex = mesh.geometry.getAttribute('skinIndex')
  const skinWeight = mesh.geometry.getAttribute('skinWeight')
  const position = mesh.geometry.getAttribute('position')
  if (position == null || skinIndex == null || skinWeight == null) return null
  const sourceSkinWeight = skinWeight.clone()
  for (let vertexIndex = 0; vertexIndex < sourceSkinWeight.count; vertexIndex += 1) {
    const values = [
      sourceSkinWeight.getX(vertexIndex),
      sourceSkinWeight.getY(vertexIndex),
      sourceSkinWeight.getZ(vertexIndex),
      sourceSkinWeight.getW(vertexIndex),
    ]
    const total = values.reduce((sum, value) => sum + value, 0)
    if (total <= 1e-8 || Math.abs(total - 1) <= 1e-8) continue
    const largestComponent = values.reduce(
      (best, value, index) => value > values[best]! ? index : best,
      0
    )
    for (let component = 0; component < 4; component += 1) {
      if (component === largestComponent) continue
      setRuntimeSkinComponent(
        sourceSkinWeight,
        vertexIndex,
        component,
    ...
    const otherWeight = [0, 1, 2, 3]
      .filter(component => component !== largestComponent)
      .reduce(
        (sum, component) => sum + getRuntimeSkinComponent(sourceSkinWeight, vertexIndex, component),
        0
      )
    setRuntimeSkinComponent(sourceSkinWeight, vertexIndex, largestComponent, 1 - otherWeight)
  }
  const source = {
    ...
    boneInverses: (mesh.skeleton.boneInverses ?? mesh.skeleton.bones.map(() => new Matrix4()))
      .map(matrix => matrix.clone()),
  }
  runtimeWingFlexSourceSkinning.set(mesh.geometry, source)
  return source
}

function deriveWingFlexSpanMaps(
  sceneRoot: Object3D,
  ...
  sceneRoot.traverse(object => {
    ...
    const position = mesh.geometry.getAttribute('position')
    const source = getRuntimeWingFlexSourceSkinning(mesh)
    if (position == null || source == null) return
    const meshToRoot = rootWorldInverse.clone().multiply(mesh.matrixWorld)
    const pointInRoot = new Vector3()

    for (const side of sides) {
      const sectionBones = side.nodes.map(entry => entry.node)
      const boneIndices = sectionBones.map(candidate =>
        source.bones.findIndex(bone => bone === candidate)
      )
      if (boneIndices.some(index => index < 0)) continue
      const boneToSection = new Map(boneIndices.map((boneIndex, sectionIndex) =>
        [boneIndex, sectionIndex] as const
      ))
      const sections: SectionStats[] = sectionBones.map(() => ({
        count: 0,
        sum: 0,
      ...
      for (let vertexIndex = 0; vertexIndex < position.count; vertexIndex += 1) {
        ...
        let rigidWeight = 0
        let valid = true
        const weightsBySection = new Map<number, number>()
        for (let component = 0; component < 4; component += 1) {
          const weight = getRuntimeSkinComponent(source.skinWeight, vertexIndex, component)
          if (weight <= 1e-5) continue
          const boneIndex = Math.round(getRuntimeSkinComponent(source.skinIndex, vertexIndex, component))
          const sectionIndex = boneToSection.get(boneIndex)
        ...
        stats.count += 1
        stats.sum += raw
        stats.min = Math.min(stats.min, raw)
        stats.max = Math.max(stats.max, raw)
      }

      if (sections.every(section => section.count > 0)) {
        candidates.get(side)!.push(sections)
      }
    }
  })

  for (const side of sides) {
    const meshSections = candidates.get(side) ?? []
    if (meshSections.length === 0 || side.nodes.length === 0) {
      side.spanMapKnots = [
        { raw: 0, mapped: 0 },
        { raw: 1, mapped: 1 },
      ]
    ...
    const centers = side.nodes.map((_, sectionIndex) => median(
      meshSections.map(sections => {
        const stats = sections[sectionIndex]!
    ...
    for (let sectionIndex = 0; sectionIndex < side.nodes.length; sectionIndex += 1) {
      knots.push({
        raw: clamp(centers[sectionIndex]!, 0, 1),
    ...
    knots.push({ raw: 1, mapped: 1 })
    const ordered = knots
      .sort((left, right) => left.raw - right.raw || left.mapped - right.mapped)
      .filter((knot, index, values) => index === 0 ||
        knot.raw > values[index - 1]!.raw + 1e-6
      )
    let previousMapped = 0
    side.spanMapKnots = ordered.map(knot => {
      const mapped = clamp(Math.max(previousMapped, knot.mapped), 0, 1)
      previousMapped = mapped
      return { raw: knot.raw, mapped }
    })
    if (side.spanMapKnots.at(-1)?.raw !== 1) {
      side.spanMapKnots = [...side.spanMapKnots, { raw: 1, mapped: 1 }]
    }
  }
}

function smoothMainWingFlexSkinWeights(
  sceneRoot: Object3D,
  sides: readonly RuntimeWingFlexSide[]
): void {
  ...
  deriveWingFlexSpanMaps(sceneRoot, sides, rootWorldInverse)
  ...
  sceneRoot.traverse(object => {
    ...
    const sourceSkinIndex = sourceSkinning.skinIndex
    const sourceSkinWeight = sourceSkinning.skinWeight

    const chains = sides.flatMap(side => {
      const bones = [side.fixedRoot, ...side.nodes.map(entry => entry.node)]
      const boneIndices = bones.map(candidate =>
        mesh.skeleton.bones.findIndex(bone => bone === candidate)
      )
      if (boneIndices.some(index => index < 0)) return []
      return [{
        side,
        boneIndices,
        boneIndexSet: new Set(boneIndices),
        spanRatios: [0, ...side.nodes.map(entry => entry.spanRatio)],
      }]
    })
    ...
    for (let vertexIndex = 0; vertexIndex < position.count; vertexIndex += 1) {
      ...
      for (let component = 0; component < 4; component += 1) {
        const weight = component === 0 ? sourceSkinWeight.getX(vertexIndex)
          : component === 1 ? sourceSkinWeight.getY(vertexIndex)
            : component === 2 ? sourceSkinWeight.getZ(vertexIndex)
              : sourceSkinWeight.getW(vertexIndex)
        if (weight <= 1e-5) continue
        activeIndices.push(component === 0 ? sourceSkinIndex.getX(vertexIndex)
          : component === 1 ? sourceSkinIndex.getY(vertexIndex)
            : component === 2 ? sourceSkinIndex.getZ(vertexIndex)
              : sourceSkinIndex.getW(vertexIndex))
      }
      if (activeIndices.length === 0) continue
      for (const candidate of chains) {
        const authoredStations = authoredStationsBySide.get(candidate.side) ?? new Set<number>()
        for (const boneIndex of activeIndices) {
          const stationIndex = candidate.boneIndices.indexOf(boneIndex)
          if (stationIndex >= 0) authoredStations.add(stationIndex)
        }
        authoredStationsBySide.set(candidate.side, authoredStations)
      }
      const chain = chains.find(candidate =>
        activeIndices.every(index => candidate.boneIndexSet.has(index))
      )
      ...
      const ratio = mapWingFlexSpanRatio(
        chain.side,
        wingSpanRatio(chain.side, pointInRoot)
      )
      let segment = chain.spanRatios.length - 2
      for (let index = 0; index < chain.spanRatios.length - 1; index += 1) {
        if (ratio <= chain.spanRatios[index + 1]!) {
          segment = index
          break
        }
      }
      const start = chain.spanRatios[segment]!
      ...
      skinIndex.setXYZW(vertexIndex, chain.boneIndices[segment]!, chain.boneIndices[segment + 1]!, 0, 0)
  ...
  for (let radius = 0; radius < WING_SURFACE_SPAN_BIN_COUNT; radius += 1) {
    ...
    for (const candidateBin of bins) {
      ...
      for (const triangleIndex of surface.spanBins[candidateBin]!) {
        ...
        if (normalHintInRoot != null) {
          triangle.getNormal(triangleNormal)
          if (Math.abs(triangleNormal.dot(normalHintInRoot)) >= 0.5 &&
            distanceSq < bestCompatibleDistanceSq) {
    ...
    // Prefer a face with compatible orientation; expand only when this span bin lacks one.
    if (bestCompatibleTriangle >= 0 ||
      (normalHintInRoot == null && checked && bestTriangle >= 0)) break
  }

  const selectedTriangle = bestCompatibleTriangle >= 0 ? bestCompatibleTriangle : bestTriangle
...
const WING_SURFACE_ATTACHMENT_MAX_DISTANCE_M = 0.05
...
function updateWingSurfaceAnchorOffsets(
  ...
  if (repair.surfaceAnchors == null) {
    ...
    for (let vertexIndex = 0; vertexIndex < repair.sourcePositions.count; vertexIndex += 1) {
      ...
      if (anchor == null ||
        anchor.distanceSq > WING_SURFACE_ATTACHMENT_MAX_DISTANCE_M ** 2) continue
  ...
  for (let vertexIndex = 0; vertexIndex < repair.sourcePositions.count; vertexIndex += 1) {
    ...
    normal.subVectors(b, a).cross(bitangent.subVectors(c, a)).normalize()
    ...
    anchors.localOffsets[offset] = delta.dot(tangent)
    anchors.localOffsets[offset + 1] = delta.dot(bitangent)
    anchors.localOffsets[offset + 2] = delta.dot(normal)
  }
}
...
function buildWingFlexAttachments(
  ...
  if (sides.length === 0) return []
  sceneRoot.updateMatrixWorld(true)
  const pivotNodes = new Set(pivotEntries.map(entry => entry.node))
  const mainNodes = new Set(sides.flatMap(side => side.nodes.map(entry => entry.node)))
  const activeSkinBoneCentroids = collectActiveSkinnedBoneCentroids(sceneRoot)
  const candidates = new Set<Object3D>(pivotNodes)
  for (const side of sides) {
    for (const child of side.fixedRoot.children) {
      collectWingFlexSkinAttachments(child, mainNodes, activeSkinBoneCentroids, candidates)
    }
  }
  ...
  return [...candidates].flatMap(node => {
    ...
    const flexSamplePointInRoot = pivotNodes.has(node)
      ? restPointInRoot
      : activeSkinBoneCentroids.get(node) ?? restPointInRoot
    const side = chooseWingFlexSide(flexSamplePointInRoot, sides)
    if (side == null) return []
    const spanRatio = mapWingFlexSpanRatio(
      side,
      wingSpanRatio(side, flexSamplePointInRoot)
    )
    ...
    return [{
      ...
      authoredQuaternionInRoot: sceneRoot.getWorldQuaternion(new Quaternion()).invert()
        .multiply(node.getWorldQuaternion(new Quaternion())),
      rotateWithWing: !pivotNodes.has(node),
    }]
  })
}

function collectActiveSkinnedBoneCentroids(sceneRoot: Object3D): ReadonlyMap<Object3D, Vector3> {
  const rootWorldInverse = sceneRoot.matrixWorld.clone().invert()
  const influences = new Map<Object3D, { pointSum: Vector3; weight: number }>()
  sceneRoot.traverse(object => {
    const mesh = object as SkinnedMesh
    if (mesh.isSkinnedMesh !== true || mesh.skeleton == null) return
    const position = mesh.geometry.getAttribute('position')
    const sourceSkinning = getRuntimeWingFlexSourceSkinning(mesh)
    if (position == null || sourceSkinning == null) return
    const skinIndex = sourceSkinning.skinIndex
    const skinWeight = sourceSkinning.skinWeight
    const meshToRoot = rootWorldInverse.clone().multiply(mesh.matrixWorld)
    const pointInRoot = new Vector3()
    for (let vertexIndex = 0; vertexIndex < skinIndex.count; vertexIndex += 1) {
      ...
      for (let componentIndex = 0; componentIndex < 4; componentIndex += 1) {
        const weight = getRuntimeSkinComponent(skinWeight, vertexIndex, componentIndex)
        if (weight <= 1e-4) continue
        const boneIndex = Math.round(getRuntimeSkinComponent(skinIndex, vertexIndex, componentIndex))
        const bone = sourceSkinning.bones[boneIndex]
        if (bone == null) continue
        const influence = influences.get(bone) ?? { pointSum: new Vector3(), weight: 0 }
        influence.pointSum.addScaledVector(pointInRoot, weight)
        influence.weight += weight
        influences.set(bone, influence)
      }
    }
  })
  return new Map([...influences].flatMap(([bone, influence]) =>
    influence.weight <= 1e-9
      ? []
      : [[bone, influence.pointSum.multiplyScalar(1 / influence.weight)] as const]
  ))
}

function getRuntimeSkinComponent(attribute: ReturnType<SkinnedMesh['geometry']['getAttribute']>, vertex: number, component: number): number {
...
function setRuntimeSkinComponent(
  attribute: ReturnType<SkinnedMesh['geometry']['getAttribute']>,
  vertex: number,
  component: number,
  value: number
): void {
  if (component === 0) attribute.setX(vertex, value)
  else if (component === 1) attribute.setY(vertex, value)
  else if (component === 2) attribute.setZ(vertex, value)
  else attribute.setW(vertex, value)
}

function collectWingFlexSkinAttachments(
  node: Object3D,
  mainNodes: ReadonlySet<Object3D>,
  activeSkinBoneCentroids: ReadonlyMap<Object3D, Vector3>,
  output: Set<Object3D>
): void {
  if (mainNodes.has(node)) {
    for (const child of node.children) {
      collectWingFlexSkinAttachments(child, mainNodes, activeSkinBoneCentroids, output)
    }
    return
  }
  if ((node as Object3D & { isBone?: boolean }).isBone === true) {
    if (activeSkinBoneCentroids.has(node)) output.add(node)
    return
  }
  for (const child of node.children) {
    collectWingFlexSkinAttachments(child, mainNodes, activeSkinBoneCentroids, output)
...
function smoothWingFlexAttachmentSkinWeights(
  ...
  for (const binding of bindings) {
    for (const side of [binding.leftWing, binding.rightWing]) {
      ...
      const attachments = binding.attachments.filter(attachment =>
      ...
      sceneRoot.traverse(object => {
        ...
        if (existingRepair != null) {
          ...
          existingRepair.cpu.useSurfaceAttachment = isStructuralSurface &&
            ...
            !existingRepair.sourceBones.some(bone => animatedAttachmentNodes.has(bone))
        ...
        const sourceSkinning = getRuntimeWingFlexSourceSkinning(mesh)
        if (position == null || sourceSkinning == null) return
        if (sourceSkinning.skinIndex.itemSize < 4 || sourceSkinning.skinWeight.itemSize < 4) return

        const sourceBoneWeights = new Map<number, number>()
        for (let vertexIndex = 0; vertexIndex < sourceSkinning.skinIndex.count; vertexIndex += 1) {
          for (let component = 0; component < 4; component += 1) {
            ...
            sourceBoneWeights.set(boneIndex, (sourceBoneWeights.get(boneIndex) ?? 0) + weight)
          }
        }
        ...
        const cpuRepair: RuntimeWingFlexSurfaceCpuRepair = {
          ...
          surfaceAnchors: null,
          useSurfaceAttachment: isStructuralSurface &&
            sourceIndices.length === 1 &&
            !hasAnimatedAttachment,
...
function isDirectWingFlexStructuralMesh(
  mesh: SkinnedMesh,
  sourceSkinning: RuntimeWingFlexSourceSkinning
): boolean {
  const boneSet = new Set<Object3D>(sourceSkinning.bones)
  const rootBone = sourceSkinning.bones.find(bone =>
    bone.parent == null || !boneSet.has(bone.parent)
  )
  return rootBone?.parent != null && mesh.parent === rootBone.parent
}
...
function addWingFlexSurfaceCpuRepair(
  ...
  repair.mesh.geometry.setAttribute('skinIndex', repair.sourceSkinning.skinIndex.clone())
  repair.mesh.geometry.setAttribute('skinWeight', repair.sourceSkinning.skinWeight.clone())
  runtimeWingFlexSourceSkinning.set(repair.mesh.geometry, repair.sourceSkinning)
  if (!binding.surfaceCpuRepairs.includes(repair)) binding.surfaceCpuRepairs.push(repair)
}
...
function runtimeMatrixChanged(left: Matrix4, right: Matrix4): boolean {
  for (let index = 0; index < 16; index += 1) {
    if (Math.abs(left.elements[index]! - right.elements[index]!) > 1e-6) return true
  }
  return false
}

function captureWingFlexSurfaceCpuPoses(
  ...
  sceneRoot.updateMatrixWorld(true)
  const rootWorldInverse = sceneRoot.matrixWorld.clone().invert()
  const authoredPoint = new Vector3()
  const matrixInRoot = new Matrix4()
  for (const repair of repairs) {
    repair.mesh.updateWorldMatrix(true, false)
    matrixInRoot.copy(rootWorldInverse).multiply(repair.mesh.matrixWorld)
    let poseChanged = !repair.authoredPoseCaptured || runtimeMatrixChanged(
      matrixInRoot,
    ...
    for (let vertexIndex = 0; vertexIndex < repair.sourcePositions.count; vertexIndex += 1) {
      authoredPoint.set(
        ...
        repair.sourcePositions.getY(vertexIndex),
        repair.sourcePositions.getZ(vertexIndex)
      )
      repair.mesh.applyBoneTransform(vertexIndex, authoredPoint).applyMatrix4(meshToRoot)
      ...
      repair.wingRatios[vertexIndex] = ratio
      const stations = [
...
function captureWingFlexAttachmentPoses(
  ...
  for (const attachment of attachments) {
    ...
    attachment.authoredQuaternionInRoot.copy(rootWorldQuaternionInverse)
      .multiply(attachment.node.getWorldQuaternion(new Quaternion()))
  }
}
...
function applyWingFlexSurfaceCpuRepairs(
  ...
  const deformedSurfaceFrames = new Map<RuntimeWingFlexSide, {
    readonly values: Float32Array
    readonly states: Uint8Array
  }>()
  ...
  const getSurfaceFrameCache = (
    side: RuntimeWingFlexSide,
    surface: RuntimeWingSurface
  ): { readonly values: Float32Array; readonly states: Uint8Array } => {
    const existing = deformedSurfaceFrames.get(side)
    if (existing != null) return existing
    const cache = {
      values: new Float32Array(surface.triangles.length * 18),
      states: new Uint8Array(surface.triangles.length),
    }
    deformedSurfaceFrames.set(side, cache)
    return cache
  }
  ...
  for (const repair of repairs) {
    ...
    const rigidBoneInverse = repair.rigidBoneIndex == null
      ? null
      : sourceSkinning.boneInverses[repair.rigidBoneIndex] ?? null
    const rigidCurrentSkinInverse = repair.rigidBoneIndex == null || rigidBoneInverse == null
      ? null
      : new Matrix4()
        .copy(mesh.bindMatrixInverse)
        .multiply(skinMatrix.multiplyMatrices(
          sourceSkinning.bones[repair.rigidBoneIndex]!.matrixWorld,
          rigidBoneInverse
        ))
        .multiply(mesh.bindMatrix)
        .invert()
    const authoredBoneMatrices = repair.rigidBoneIndex == null
      ? sourceSkinning.bones.map((_, boneIndex) => {
      ...
      : null
    const currentBoneMatrices = repair.rigidBoneIndex == null
      ? sourceSkinning.bones.map((bone, boneIndex) => {
        const boneInverse = sourceSkinning.boneInverses[boneIndex]
        return boneInverse == null
          ? new Matrix4()
    ...
    if (rigidCurrentSkinInverse != null) {
      ...
      const surface = repair.side.surface
      const anchors = repair.surfaceAnchors
      const frameCache = surface != null && anchors != null
        ? getSurfaceFrameCache(repair.side, surface)
        : null
      for (let vertexIndex = 0; vertexIndex < position.count; vertexIndex += 1) {
        const offset = vertexIndex * 3
        let rootX: number
        ...
        const surfaceTriangle = surface != null && triangleIndex !== 0xffffffff
          ? surface.triangles[triangleIndex]
          : undefined
        if (surface != null && surfaceTriangle != null && anchors != null && frameCache != null) {
          const frameOffset = triangleIndex * 18
          let frameState = frameCache.states[triangleIndex]!
          if (frameState === 0) {
            deformSurfaceVertex(repair.side, surface, field, surfaceTriangle.a, surfaceA)
            deformSurfaceVertex(repair.side, surface, field, surfaceTriangle.b, surfaceB)
            deformSurfaceVertex(repair.side, surface, field, surfaceTriangle.c, surfaceC)
            const ax = surfaceA.x
            const ay = surfaceA.y
            const az = surfaceA.z
            const abx = surfaceB.x - ax
            const aby = surfaceB.y - ay
            const abz = surfaceB.z - az
            const acx = surfaceC.x - ax
            const acy = surfaceC.y - ay
            const acz = surfaceC.z - az
            let tx = abx
            let ty = aby
            let tz = abz
            ...
            if (tangentLength <= 1e-9 || normalLength <= 1e-9) {
              frameCache.states[triangleIndex] = 2
              frameState = 2
            } else {
              tx /= tangentLength
              ty /= tangentLength
              ...
              by /= bitangentLength
              bz /= bitangentLength
              const values = frameCache.values
              values[frameOffset] = ax
              values[frameOffset + 1] = ay
              values[frameOffset + 2] = az
              values[frameOffset + 3] = abx
              values[frameOffset + 4] = aby
              values[frameOffset + 5] = abz
              values[frameOffset + 6] = acx
              values[frameOffset + 7] = acy
              values[frameOffset + 8] = acz
              values[frameOffset + 9] = tx
              values[frameOffset + 10] = ty
              values[frameOffset + 11] = tz
              values[frameOffset + 12] = bx
              values[frameOffset + 13] = by
              values[frameOffset + 14] = bz
              values[frameOffset + 15] = nx
              values[frameOffset + 16] = ny
              values[frameOffset + 17] = nz
              frameCache.states[triangleIndex] = 1
              frameState = 1
            }
          }
          if (frameState === 2) {
          ...
          const wb = anchors.barycentrics[offset + 1]!
          const wc = anchors.barycentrics[offset + 2]!
          const localU = anchors.localOffsets[offset]!
          const localV = anchors.localOffsets[offset + 1]!
          const localN = anchors.localOffsets[offset + 2]!
          const anchorX = values[frameOffset]! + values[frameOffset + 3]! * wb + values[frameOffset + 6]! * wc
          const anchorY = values[frameOffset + 1]! + values[frameOffset + 4]! * wb + values[frameOffset + 7]! * wc
          const anchorZ = values[frameOffset + 2]! + values[frameOffset + 5]! * wb + values[frameOffset + 8]! * wc
          rootX = anchorX + values[frameOffset + 9]! * localU + values[frameOffset + 12]! * localV + values[frameOffset + 15]! * localN
          rootY = anchorY + values[frameOffset + 10]! * localU + values[frameOffset + 13]! * localV + values[frameOffset + 16]! * localN
          rootZ = anchorZ + values[frameOffset + 11]! * localU + values[frameOffset + 14]! * localV + values[frameOffset + 17]! * localN
        } else {
          const x = repair.authoredPositionsInRoot[offset]!
          const y = repair.authoredPositionsInRoot[offset + 1]!
          const z = repair.authoredPositionsInRoot[offset + 2]!
...
function sampleWingFlex(
  ...
  // Euler-Bernoulli cantilever under distributed load. The entire wing participates
  // in one continuous curve instead of starting at an authored helper joint.
