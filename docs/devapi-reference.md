...
- `interactions.active()` returns each running or held target with operation, source, lifecycle, start time, and stop status. `stop(target)` and `stopAll()` use the same idempotent lifecycle: a held control runs its authored release route, then unlocks if captured, before feedback and timers are cleared. Stopping leaves the control at its current state and never rolls it back.
...
- `interactions.profiles.list/get/effective` inspect the version 2 profile store. Profiles map mouse and wheel inputs plus the `shortcuts.stop` keyboard shortcut, which defaults to the layout-stable `KeyboardEvent.code` `Escape`. `create`, `duplicate`, `rename`, `delete`, and `reset` manage profiles; `selectGlobal` and `selectAircraft(packageRoot, aircraftId, profileIdOrNull)` select them; `export` and validated `import` round-trip the complete store.
