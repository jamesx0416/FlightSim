...
| `cockpitTextures` | `?cockpitTextures=range-low` | Selects cockpit/interior-view texture loading. If omitted, the viewer uses `range-low`, which fetches DDS headers first, then requests only the selected small mip byte range when the server supports HTTP `Range`; decoded normal/transparent DDS paths also use range-loaded low mips. This keeps button caps, labels, panels, and decals textured without downloading full cockpit atlases. Use `?cockpitTextures=full` to request the normal full DDS path. If range loading fails or ranges are unsupported, the loader falls back to the full DDS path rather than committing a permanent placeholder. |
...
| `ddsDebug` | `?ddsDebug` | Logs DDS range-low placeholder fallbacks to the console for texture loading diagnostics. Without this flag, expected range fallback noise is suppressed so startup console output remains focused on actionable loader/runtime issues. |
...
| `vcockpitGaugeMode` | `?vcockpitGaugeMode=video` | Selects how non-WASM HTML gauges are displayed. `texture` is the default and composites iframe DOM into the VCockpit `CanvasTexture`. `overlay` is experimental direct HTML over projected screen bounds. `video` is experimental and keeps gauges on the mesh using a canvas `captureStream()` plus Three `VideoTexture`; it still rasterizes HTML into the surface canvas, but lets the browser/video texture path schedule frame upload. `htmlTexture` is query-only experimental: the loader feature-detects native browser HTML texture hooks and currently falls back to `texture` with diagnostics until native renderer integration and long-session validation are complete. |
| `vcockpitGaugeOverlayProjection` | `?vcockpitGaugeMode=overlay&vcockpitGaugeOverlayProjection=quad` | Selects overlay iframe placement. `bounds` keeps the previous axis-aligned object-bounds behavior. `quad` experimentally projects the VCockpit screen UV quad to the viewport and warps the iframe with CSS `matrix3d()`. Occlusion/depth sorting and pointer routing are not complete. |
...
| `vcockpitGaugeCaptureFps` | `?vcockpitGaugeCaptureFps=10` | Caps dirty HTML gauge DOM rasterization and texture updates for texture/video modes. `vcockpitGaugeCaptureHz` is accepted as an alias. Values are clamped from `1` to `60`; default is `8`. |
| `vcockpitGaugeRasterScale` | `?vcockpitGaugeRasterScale=0.5` | Opts texture/video VCockpit gauges into lower-resolution dynamic textures while keeping hidden gauge iframes at their authored panel dimensions. Values are clamped from `0.25` to `1`; default is `0.75`. Lower values reduce canvas draw and texture upload cost, but may blur screens. Overlay mode ignores this and uses `1`. |
| `vcockpitGaugeUpdateOutside` | `?vcockpitGaugeUpdateOutside=on` | Debug override for exterior view. By default, VCockpit gauge iframes stay mounted for behavior bridge state but do not run instrument updates or trigger texture captures outside cockpit view. Set this to `on` to keep outside-view gauge updates and captures active for diagnostics. The same toggle is available in the viewer settings panel as `Gauge Updates Outside`. |
| `vcockpitGaugeUpdateMs` | `?vcockpitGaugeUpdateMs=125` | Forces generic iframe-hosted instrument `Update()` calls to run periodically using a minimum interval in milliseconds. Values are clamped from `16` to `5000` ms. Omit the parameter to align texture/htmlTexture gauge updates with `vcockpitGaugeCaptureFps`; set it to `0` or `off` to update every animation frame. |
| `vcockpitGaugeUpdateHz` | `?vcockpitGaugeUpdateHz=8` | Alternative to `vcockpitGaugeUpdateMs`: forces generic iframe-hosted instruments into periodic `Update()` calls by frequency. Ignored when `vcockpitGaugeUpdateMs` is set. Values are clamped from `0.2` to `60` Hz. Omit both update parameters to use the texture capture cadence. |
...
| `cockpitInteractionHitboxes` | `?cockpitInteractionHitboxes` | Shows diagnostic cockpit interaction picking geometry. Resolved behavior targets highlight their actual pickable meshes with a translucent green wireframe overlay; behavior targets with no renderable mesh descendants show small red fallback boxes. The setting is also available in the viewer settings panel as `Interaction Hitboxes`. |
| `skipGaugeSettingSeed` | `?skipGaugeSettingSeed` | Skips the load-time gauge/EFB setting preseed entirely. By default, the viewer only pre-seeds settings that already exist in browser `localStorage` and never applies aircraft JS `defaultValue` declarations as runtime state. The same toggle is available in the viewer settings panel as `Seed Nothing`; it is off by default and takes effect on the next load. |
...
| `FLIGHTSIM_AIRCRAFT_CACHE_MODE` | Controls local package-asset caching for `/aircrafts/<package>/**` and `/vendor/msfs-stock/**`. Supported values are `normal`, `no-store`, `no-cache`, and `immutable`. `normal` leaves server defaults unchanged. `no-store` disables storage. `no-cache` stores responses but revalidates them. In `immutable` mode the viewer checks one small `__asset-version.json` per package root, adds that revision to package asset URLs, and serves only matching versioned URLs with long-lived immutable headers; unversioned or outdated URLs remain `no-cache`. XML and other complete files use the browser HTTP cache. DDS byte ranges use IndexedDB because browser caching of `206` responses is unreliable, and `.FLAGS` requests are skipped when the authoritative package layout does not contain the file. A package revision is derived from sorted file path, size, and modification-time metadata and is invalidated by Vite file-watcher events. Diagnostics and maintenance methods are under `window.__DevApi.assetCache`. |
