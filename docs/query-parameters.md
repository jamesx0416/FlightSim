...
| `cockpitInteractionHitboxes` | `?cockpitInteractionHitboxes` | Shows diagnostic green wireframe boxes for resolved cockpit interaction targets. These are behavior-derived click hitboxes, built from the loaded cockpit nodes that match compiled `NODE_ID`, component node, or `ANIM_NAME` interaction bindings. The setting is also available in the viewer settings panel as `Interaction Hitboxes`. |
