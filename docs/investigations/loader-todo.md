...
- Current first-slice implementation:
  ...
  - bridge-first support now creates a synthetic generic host for missing `WasmInstrument/WasmInstrument.html?...` entries on bound VCockpit surfaces, so WASM-backed HTML gauges can load as `loaded-wasm-bridge` and use the existing SimVar/key/HTML-event bridge instead of failing as unresolved host assets. Native MSFS WASM ABI execution is still explicitly diagnosed as unsupported.
    ...
    - A320 fixture smoke verification on the matching cockpit route returned zero errors and no active WASM gauge entries in that sample; screenshot captured to `backups/agent-browser/wasm-bridge-support/a320-wasm-bridge-smoke.png`.
    - attempted headless loading for `texture=no_texture` backend WASM surfaces was intentionally backed out in this pass after the A330 Agent Browser verification hung; backend-only WASM hosts need a separate bounded scheduler/runtime design before enabling them.
