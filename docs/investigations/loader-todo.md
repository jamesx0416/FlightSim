...
- [ ] `msfs2024/html/3_Models_And_Textures/Plugins/glTF_Schemas.htm`
  - Reviewed against the mirrored SDK schema index on 2026-05-28. The loader handles the in-scope built-aircraft glTF subset that is active in the A330/A320 routes: `MSFT_texture_dds`, DirectX normal-map conversion / `ASOBO_normal_map_convention` behavior, `ASOBO_material_blend_gbuffer`, `ASOBO_material_detail_map`, `ASOBO_material_draw_order`, `ASOBO_material_invisible`, `ASOBO_material_shadow_options`, material-code extras, primitive base-vertex/winding normalization, skinning/texcoord/color normalization, and property-animation/runtime XML behavior through the model XML path. The remaining schema index entries are explicitly not fully implemented yet: collision/gizmo/macro-light authoring extensions, extra occlusion, alpha dither, anisotropic/clearcoat/dirt/environment-occluder/fake-terrain/fresnel/ghost/glass/parallax/pearlescent/rain/sail/SSS/UV/wind-flex/windshield material extensions, softbody meshes, and native simulator collision/effect semantics. Keep this item unchecked until each in-scope schema is either implemented and route-verified or explicitly split into checked/blocked subitems.
- [ ] `msfs2024/flighting/html/3_Models_And_Textures/Textures/Materials/FlightSim_Materials.htm`
  - Reviewed against the mirrored SDK material-type overview on 2026-05-28. Current generic support covers standard PBR loading, opaque/mask/blend alpha handling, DXT/DDS texture fallback, porthole alpha transparency, invisible materials, shadow no-cast metadata, MSFS geometry-decal / blend-gbuffer ordering, sibling receiver projection, forward color/component masking, decal depth masking where supported by the renderer, detail color/normal/metal-rough-AO/blend-mask maps, DirectX normal conversion, draw order, and generic double-sided/texture transform behavior inherited from glTF/Three. The full SDK page remains broader than the viewer: windshield rain/wiper/iridescent behavior, clearcoat varnish layering, parallax interiors, anisotropic/hair/SSS/fresnel/environment-occluder/ghost/sail/propeller/windflex/tree/vegetation/tire material families, collision material gameplay, road/ground collision materials, day-night cycle, motion blur, wear overlays, and simulator-specific terrain/scenery semantics are not fully implemented. Keep this item unchecked until those unsupported material families are either implemented generically or split out as blocked/out-of-scope contracts.
- [ ] `msfs2024/html/3_Models_And_Textures/Textures/Materials/FlightSim_Material_Parameters.htm`
  - Reviewed against the mirrored SDK parameter list on 2026-05-28. The loader currently consumes the decal per-component blend factors exported through `ASOBO_material_blend_gbuffer`, applies normal blend scaling, supports detail-map UV scale/offset and blend-threshold inputs, preserves material-code metadata, applies porthole alpha behavior, and uses draw-order/decal material state in the renderer. Parameters that require dedicated shader models or simulator systems remain unsupported: pearl color shift, windshield scratches/refraction/rain/iridescence/wipers/debug masks, clearcoat factors/textures, parallax room parameters, glass thickness optics, SSS color, fresnel/ghost/sail/windflex/vegetation/tire parameters, decal normal override mode, clearcoat render-under/over behavior, and geodecal-blended sharpness semantics. Keep this item unchecked until unsupported parameters are implemented generically or explicitly split into blocked subitems.
