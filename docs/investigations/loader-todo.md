...
- [x] Preserve stock `MouseRect` default/drag interaction-mode callback code.
  ...
  - Blocked for rendered interaction parity on 2026-05-27: stock `ASOBO_GT_Interaction_Blocker` / `ASOBO_GT_Interaction_NoInteraction` expands cleanly with zero diagnostics, but its simulator meaning is a disabled `MouseRect` that blocks hit-testing rather than executable callback RPN. The current behavior IR only represents executable interaction callbacks/events, so there is no generic disabled-MouseRect/hit-test-blocker runtime surface to preserve this semantics without adding a new interaction-blocker representation.
