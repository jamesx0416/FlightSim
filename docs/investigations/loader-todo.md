...
- [x] Implement generic model-level `NodeAnimation` runtime support from official docs.
  ...
  - 2026-05-29 pedestal long-session follow-up: a VCockpit offscreen-surface suspension experiment was rejected because it optimized the look-away case instead of the visible pedestal/MCDU symptom. Repeated `SimVar.GetRegisteredId` calls inside gauge frames now hit a local fast cache instead of mutating bridge diagnostics on every cached lookup, which targets visible gauge update work without changing gauge active/inactive behavior.
  - 2026-05-29 visible pedestal/MCDU follow-up: generic VCockpit `unknown` dirty events from bridge/runtime notifications no longer force full static DOM/SVG/text recapture. DOM mutations still refresh the static layer, canvas writes still redraw dynamic canvases, and unknown runtime events reuse the cached static layer unless a real DOM/canvas change is observed.
