...
- [x] Implement generic model-level `NodeAnimation` runtime support from official docs.
  ...
  - 2026-05-29 pedestal long-session follow-up: the VCockpit runtime now keeps initial full gauge capture intact, then suspends texture-surface gauge `Update()` loops and dirty captures when the bound cockpit surface is outside the current viewport for a short grace period. Returning the surface to view reactivates the gauge and marks it dirty for a fresh capture. Repeated `SimVar.GetRegisteredId` calls inside gauge frames also hit a local fast cache instead of mutating bridge diagnostics on every cached lookup. This is generic panel-surface visibility and bridge-cache behavior; it does not use aircraft, gauge, or material names.
