...
- see [NOTES.md](../../NOTES.md) for rejected transform experiments and durable material/decal observations
...
- the A32NX flap/wing decal clipping is a separate renderer/material issue; detailed evidence is in [NOTES.md](../../NOTES.md)
...
- Current first-slice implementation:
  ...
  - Add an experimental `?vcockpitGaugeMode=htmlTexture` path only when native browser HTML-in-Canvas support is available:
    ...
    - 2026-05-27 update: the query value is now accepted only as an explicit experimental request. The loader feature-detects `drawElementImage`, `texElementImage2D`, and WebGPU `copyElementImageToTexture`, records support details as a diagnostic, and keeps the effective renderer on the dirty-driven `CanvasTexture` path until native integration and long-session A330/A320 verification prove the path is safe.
  ...
  - HTML gauge capture defaults to live refresh with generic instrument `Update()` scheduling aligned to the texture capture cadence, with `?vcockpitGaugeUpdateMs=` / `?vcockpitGaugeUpdateHz=` available for forced periodic iframe updates, `?vcockpitGaugeUpdateMs=off` available for every-animation-frame diagnostics, and `?vcockpitLiveGauges=off` available for a bounded first-successful-frame pass that caches captured pixels and releases hidden iframes.
...
- [x] Support right-button cockpit pointer drags through the generic interaction path.
  - Cockpit pointer handling now accepts right mouse button pointerdown/move/up while cockpit view is active, suppresses the browser context menu, and emits stock `LeftDrag` callback phases with generic `M:RelativeX/Y/Z`, `M:DragPercent`, and `M:InputType=1` values when the held target has a drag callback.
  - Verified with `tsc --noEmit` and Agent Browser on 2026-05-14 using the A330 LOD0 cockpit route: `__DevApi.input.pointer({ type: "down", button: 2, ... })` over `PUSH_AUTOPILOT_CSTR_L` hit the cockpit mesh, set `activeHeldTarget`, `pointermove` kept the held interaction without rotating the view, `pointerup` released it, a synthetic `contextmenu` event was default-prevented, and diagnostics stayed at zero errors / zero gauge issue groups.
- [x] Compile structured stock drag callback nodes.
  ...
  - Follow-up verification on 2026-05-27 mounted the real `Asobo/Generic/Interactions.xml` and exercised `ASOBO_GT_Interaction_DraggingXAxis_EventsIncDec` plus `ASOBO_GT_Interaction_DraggingYAxis_EventsIncDec`. Both stock templates compiled their full `CallbackJumpDragging` payloads with zero diagnostics; selected runtime-equivalent callback evaluation emitted `TEST_X_INC` / `TEST_X_DEC` and `TEST_Y_INC` / `TEST_Y_DEC` for relative drags, plus wheel up/down events on the Y variant.
- [x] Preserve stock `MouseRect` default/drag interaction-mode callback code.
  ...
  - Follow-up support covers stock `Loop` `Then` expansion and selected `MouseRect` `Switch/Case` payloads, which the mounted `ASOBO_GT_MouseRect` implementation uses to emit `CallbackCode`, `EventID`, `CallbackDragging`, and `CallbackJumpDragging` after interaction-mode parameter processing. Verified with `tsc --noEmit` and Chrome DevTools MCP on 2026-05-27: a synthetic component mounted the real `Asobo/Generic/Interactions.xml` plus `Asobo/Generic/Subtemplates/Interactions_Subtemplates.xml`; `ASOBO_GT_Interaction_DraggingYAxis_SimGates` compiled the full gate-aware callback with zero diagnostics, `LeftSingle` initialized `O:MaxDraggingPosition` / `O:MinDraggingPosition`, `LeftDrag` emitted `K:TEST_GATE_SET`, and normal target lookup selected the gate-aware callback over the older simple fallback. A330 and A320 exterior-only route smokes both loaded with zero diagnostics; screenshots captured to `backups/agent-browser/generic-interaction-loop-support/`.
  - Follow-up verification on 2026-05-27 also exercised `ASOBO_GT_Interaction_DraggingYAxis_Gates` through the real mounted stock XML. The selected full callback contained the stock gate limit code and custom `DRAG_CODE`, compiled with zero diagnostics, and runtime-equivalent evaluation of `Lock` / `LeftDrag` / `Unlock` updated `O:MaxDraggingPosition`, `O:MinDraggingPosition`, `O:IsDragging`, and the target `L:` position.
  - Follow-up verification on 2026-05-27 mounted the full stock layout and exercised tooltip/emissive-adjacent `MouseRect` paths with zero diagnostics: `ASOBO_GT_Interaction_Tooltip` with `EMISSIVE_CODE` compiled a material emissive binding for the component target, and `ASOBO_GT_MouseRect` with `EMISSIVE_NODE_POTENTIOMETER` compiled the stock light-potentiometer material binding. Tooltip display metadata remains preserved only as stock XML data during traversal; this viewer does not yet implement a user-facing cockpit tooltip renderer.
  - Resolved for rendered interaction parity on 2026-05-28: stock `ASOBO_GT_Interaction_Blocker` / `ASOBO_GT_Interaction_NoInteraction` now compiles disabled `MouseRect` semantics into generic interaction-blocker records when `DISABLE_MOUSERECT=True`. The runtime exposes those records through `AircraftRuntime.getInteractionBlockers()`, cockpit picking resolves them to blocker meshes/hitboxes, and nearer blockers prevent executable interactions without creating fake callback RPN.
- [x] Preserve stock left/leave/wheel mouse-event callback code.
  ...
  - Follow-up coverage on 2026-05-14 mounted the real stock `Asobo/Generic/Interactions.xml` and `Asobo/Generic/Subtemplates/Interactions_Subtemplates.xml` in an in-page compiler/runtime harness. Representative `LeftSingle_Event`, `LeftSingle_Code`, `LeftSingle_Leave_Code`, `LeftSingle_MultiStates`, `DraggingYAxis_EventSet`, and `WheelLeftDrag_Axis` components compiled to 6 interaction bindings with zero diagnostics; runtime execution produced the expected `K:TEST_EVENT_KEY`, `K:TEST_AXIS_SET`, `K:TEST_INC` / `K:TEST_DEC` events and `L:` values for left, leave, wheel, multistate, and axis paths.
  - Follow-up verification on 2026-05-27 mounted the real stock `Asobo/Generic/Interactions.xml` and exercised `ASOBO_GT_Interaction_WheelAndContinuousLeft_LeftSingle` plus `ASOBO_GT_Interaction_WheelAndContinuousLeft_PushPull`. Both templates compiled with zero diagnostics into callback and stock 10 Hz update bindings; runtime-equivalent evaluation exercised wheel up/down, center push, push/pull, leave, and drag threshold paths, writing the expected `L:` values and generic `O:` lock/repeat state.
  - Follow-up verification on 2026-05-27 exercised `ASOBO_GT_Interaction_WheelAndContinuousLeft_LeftSingleTimed` with the same mounted stock XML. The richer timed callback and long-press update compiled with zero diagnostics; short release wrote `L:WHEEL_TIMED_SHORT` and cleared `O:_PressTime`, while holding past `LONG_CLICK_TIME=1` wrote `L:WHEEL_TIMED_LONG` and left release only ran the leave path.
  - Follow-up verification on 2026-05-27 exercised `ASOBO_GT_Interaction_WheelLeftSingle_Axis` and `ASOBO_GT_Interaction_DraggingYAxis_Code` through mounted stock XML. Both compiled with zero diagnostics; runtime-equivalent evaluation emitted positive/negative axis writes, drag up/down writes, press/release writes, and target position updates through generic callback RPN.
  - Follow-up verification on 2026-05-27 exercised wrapper coverage for `ASOBO_GT_Interaction_WheelAndContinuousLeft` horizontal/vertical dispatch and `ASOBO_GT_Interaction_DraggingXAxis_Code`. Mounted stock XML compiled seven interactions and two updates with zero diagnostics; runtime-equivalent evaluation wrote horizontal/vertical wheel values and X-axis drag up/down position changes.
- [x] Preserve stock default timed left-click interaction updates.
  - Stock `ASOBO_GT_Interaction_Left_Timed_Code` default timing compiles both the callback press/release binding and its 10 Hz `Update` binding. The update path handles long-press completion by clearing `O:_PressTime`, while the release path still handles short click release and leave code.
  - Verified with an Agent Browser in-page compiler/runtime harness on 2026-05-14 using the real mounted `Asobo/Generic/Interactions.xml`: a synthetic timed component produced one callback binding and one update binding with zero diagnostics; short press/release wrote `L:TIMED_RELEASE=4`, `L:TIMED_SHORT=2`, and `L:TIMED_LEAVE=5`, while holding past `LONG_CLICK_TIME=1` wrote `L:TIMED_LONG=3` and then release wrote only `L:TIMED_LEAVE=5`.
  - Follow-up verification on 2026-05-27 covered the `TIMED_TYPE=X_EVENTS` branch through mounted stock XML. The callback and 10 Hz update compiled with zero diagnostics; short release wrote release, short, and leave state, holding past `LONG_CLICK_TIME_1` wrote the first long action and advanced `_Process_Step`, and holding past `LONG_CLICK_TIME_2` wrote the second long action and cleared `_PressTime`.
...
- [x] Preserve generic state for unbound `B:` button, switch, knob, and lever events.
  ...
  - Follow-up runtime support mirrors generic APU bleed local switch writes into simulator-facing bleed variables. Cockpit packages can drive APU bleed through local `L:` pushbutton state instead of stock `K:APU_BLEED_AIR_SOURCE_*` events; the runtime now detects generic APU+BLEED local switch names and updates `A:BLEED AIR APU` plus `A:BLEED AIR SOURCE CONTROL` without aircraft-specific node or package aliases.
    - Verified on 2026-05-31 with `tsc --noEmit` and Chrome DevTools `window.__DevApi` smoke on A330 and A320 cockpit routes: cold-and-dark startup clicked battery, external power, APU master/start/generator/bleed, fuel pumps, engine mode, and engine masters. Both routes stayed at zero error diagnostics, loaded 24/24 gauges with 18/18 captured capturable gauges, and reached `A:APU PCT RPM = 100`, `A:BLEED AIR APU = 1`, `A:BLEED AIR SOURCE CONTROL = 1`, fuel valves 1/2 open, starters 1/2 on, combustion 1/2 on, and `A:TURB ENG N2:1/2 = 55`; screenshots captured to `backups/agent-browser/systems-smoke-audit/`.
  ...
  - Follow-up MCDU/FMC brightness event support applies the stock `Asobo/Airliner/FMC.xml` BRT/DIM increment contract when airliner CDU HTML events such as `A320_Neo_CDU_1_BTN_BRT` / `..._DIM` are dispatched. The side effect updates `I:XMLVAR_MCDU_#ID#_Brightness` with the stock 0.03 step and 0.05..1 clamp, so package-authored CDU H-event cycles can move the same generic brightness variable as stock input-event BRT/DIM bindings.
    - Verified with `tsc --noEmit`, a local Bun runtime harness, and Agent Browser on A330 and A320 LOD0 cockpit routes on 2026-05-14: direct runtime events moved MCDU brightness `0.50 -> 0.53 -> 0.50`; live `PUSH_MCDUL_BRT` cockpit targets emitted the expected BRT/DIM HTML events and moved `I:XMLVAR_MCDU_1_Brightness` `0.50 -> 0.53 -> 0.50` on both aircraft. A330 stayed at 24/24 loaded gauges with 18/18 captured capturable gauges; A320 stayed at 24/24 loaded gauges with 18/18 captured capturable gauges; both routes kept zero gauge issue groups and zero error diagnostics.
  - Follow-up runtime support publishes the base `B:` held state for handled generated input-event bindings that end in `_Push`, `_Release`, `_On`, or `_Off` before evaluating their compiled binding source. This covers stock templates whose handled momentary callbacks also drive update loops by reading the unsuffixed base input-event state.
    - Verified with `tsc --noEmit` and an in-browser synthetic stock harness on 2026-05-14: mounting `Asobo/Airliner/FMC.xml` and expanding both `ASOBO_FMC_Push_BRT_DIM_Template` branches produced zero diagnostics; the 3-state path moved MCDU brightness `0.50 -> 0.53 -> 0.50`; the split held-button path published `B:FMC_TEST_2_Button_BRT_Solo = 1 -> 0` across push/release, moved brightness upward while BRT was held, and moved it downward while DIM was held.
...
- [x] `Content_Configuration/Cameras/Cameras_CFG/cameras_cfg.htm`
  - Reviewed against the mirrored SDK reference on 2026-05-28. The loader resolves `cameras.cfg` from aircraft/base-container directories, parses `[VIEWS].eyepoint`, and selects documented `[CAMERADEFINITION.N]` cockpit/external camera entries from `Origin`, `Category`, `SubCategory`, `SubCategoryItem`, `Title`, `InitialXyz`, and `InitialPbh`; cockpit bank is ignored for cockpit-origin cameras per the SDK note. Simulator UI-only camera behavior such as momentum, panning rates, ray-node collision, per-camera variable toggles, and target-instance cameras remains outside the current viewer scope. Verified with `tsc --noEmit` and Agent Browser A330/A320 cockpit route smokes; both entered cockpit from the cameras.cfg-backed pose with zero error diagnostics and screenshots were captured under `backups/agent-browser/cameras-cfg-support/`.
...
- [x] `Content_Configuration/SimObjects/Aircraft_SimO/Aircraft.htm`
  - Reviewed against the mirrored SDK overview on 2026-05-28. The loader follows the documented aircraft SimObject folder split by discovering `aircraft.cfg`, root aircraft CFG files, `model*/model.cfg`, `panel*/panel.cfg`, `sound*/sound.xml`, and `texture*/texture.cfg` through layout entries and documented suffixes; the referenced datum/physics, legacy `.air`, and legacy effect authoring behavior are simulator systems outside the current visual loader scope. Covered by the checked CFG targets below plus A330/A320 route smokes under `backups/agent-browser/aircraft-cfg-support/`, `model-cfg-support/`, `texture-cfg-fallback-support/`, and `cameras-cfg-support/`.
...
- [x] `msfs2024/html/3_Models_And_Textures/Plugins/glTF_Schemas.htm`
  ...
  - Blocked/out of current aircraft-loader scope as of 2026-05-28: collision/gizmo/macro-light authoring extensions, softbody meshes, native simulator collision/effect semantics, and material extensions that require dedicated simulator shader systems rather than generic built-aircraft loading. Those unsupported families must remain separate future contracts rather than blocking the covered aircraft glTF ingestion subset.
- [x] `msfs2024/flighting/html/3_Models_And_Textures/Textures/Materials/FlightSim_Materials.htm`
  - Reviewed against the mirrored SDK material-type overview on 2026-05-28. Current generic support covers standard PBR loading, opaque/mask/blend alpha handling, DXT/DDS texture fallback, porthole alpha transparency, invisible materials, shadow no-cast metadata, MSFS geometry-decal / blend-gbuffer ordering, sibling receiver projection, forward color/component masking, decal depth masking where supported by the renderer, detail color/normal/metal-rough-AO/blend-mask maps, DirectX normal conversion, draw order, and generic double-sided/texture transform behavior inherited from glTF/Three. Verified through the A330/A320 material, decal, texture fallback, and route-smoke evidence recorded above.
  - Blocked/out of current aircraft-loader scope as of 2026-05-28: windshield rain/wiper/iridescent behavior, clearcoat varnish layering, parallax interiors, anisotropic/hair/SSS/fresnel/environment-occluder/ghost/sail/propeller/windflex/tree/vegetation/tire shader families, collision material gameplay, road/ground collision materials, day-night cycle, motion blur, wear overlays, and simulator terrain/scenery semantics. These require dedicated generic shader/simulator systems before they can be checked as runtime material support.
- [x] `msfs2024/html/3_Models_And_Textures/Textures/Materials/FlightSim_Material_Parameters.htm`
  - Reviewed against the mirrored SDK parameter list on 2026-05-28. The loader consumes the decal per-component blend factors exported through `ASOBO_material_blend_gbuffer`, applies normal blend scaling, supports detail-map UV scale/offset and blend-threshold inputs, preserves material-code metadata, applies porthole alpha behavior, and uses draw-order/decal material state in the renderer. Verified through the A330/A320 material, decal, texture fallback, and route-smoke evidence recorded above.
  - Blocked/out of current aircraft-loader scope as of 2026-05-28: pearl color shift, windshield scratches/refraction/rain/iridescence/wipers/debug masks, clearcoat factors/textures, parallax room parameters, glass thickness optics, SSS color, fresnel/ghost/sail/windflex/vegetation/tire parameters, decal normal override mode, clearcoat render-under/over behavior, and geodecal-blended sharpness semantics. These parameters need dedicated generic shader support or simulator subsystems and must not be approximated with aircraft-specific heuristics.
- [x] `msfs2024/flighting/html/6_Programming_APIs/SimVars/Aircraft_SimVars/Aircraft_FlightModel_Variables.htm`
- [x] `html/mergedProjects/How_To_Make_An_Aircraft/Contents/Modelling/Airframe/Texturing/Surface_Detail__agt_index.htm`
  - Reviewed against the mirrored aircraft surface-detail guidance on 2026-05-28. The in-scope runtime pieces are covered generically: DirectX normal maps are converted for the renderer, geometry decals use `ASOBO_material_blend_gbuffer`, draw order is derived from `ASOBO_material_draw_order` plus primitive/material order, decal component blend factors are consumed for color/roughness/metal/occlusion/normal/emissive handling, detail maps cover color/normal/ORM/blend-mask texture overlays, and the same-mesh projection/depth-mask path handles surface rivet/livery/text decals without aircraft-name exceptions. Remaining guidance in that page is authoring advice for deciding when to model geometry versus bake normals, and does not add a separate runtime contract.
...
- [x] Implement generic model-level `NodeAnimation` runtime support from official docs.
  ...
  - 2026-05-03: rejected transform/depth-bias experiments and A32NX/A330 flap material comparison notes were moved to [NOTES.md](../../NOTES.md).
  ...
  - 2026-05-03 deeper follow-up: depth masking fixes only visibility/occlusion; see [NOTES.md](../../NOTES.md) for the durable material finding behind the same-mesh decal projection fallback.
