...
- [x] Complete direct browser acceptance for hit testing and replacement paths. Interaction meshes, fallback hitboxes, passive gauges, LOD replacement, live target disappearance, a real mounted blocker, and an authored cover are verified. A physical `KNOB_CHRONO_GPS` click was consumed as a blocker with no camera motion. Authored `COVER_NODE_ID`/`LOCK_NODE_ID` relationships are now preserved; a physical click on the mounted A339X `LOCK_OVHD_CALLS_EMER` guard selected and executed the guard interaction mesh rather than the protected control or camera.
- [x] Run direct-mouse and DevApi acceptance across the complete representative A330 control matrix in both Legacy and Lock, including presentation and exact controls. Mounted DevApi acceptance covers button, momentary, reversible toggle, finite and infinite detents, primary/secondary/tertiary compound knob behavior, guarded/covered controls, inner/outer RMP tuning plus transfer, FCU baro push/pull and wheel routes, exact flap Set with restoration, camera arbitration, fallback hitboxes, and lifecycle paths. Legacy correctly rejects secondary/tertiary compound routes authored only for the drag/Lock model. The final physical presentation sweep hovered the mounted `LOCK_OVHD_CALLS_EMER` mesh with the real browser mouse in both modes: the viewer showed its title, current value, and `Press · Lock` actions, and Lock mode applied the authored blue interaction highlight.
...
- [x] Complete the compiler/control matrix for every channel, phase, callback kind, EventID/InputEvent route, compound control, timing mode, precedence rule, and unsupported MSFS 2024 diagnostic. Focused coverage now asserts the complete supported MouseRect event matrix across primary, secondary, tertiary, drag, double, release, wheel, repeat, hover/leave, lock/unlock, plus callback code, EventID, InputEvent, CallbackDragging, and CallbackJumpDragging source kinds; existing tests cover compound arbitration, timing, precedence, typed parameters, and fail-closed unsupported diagnostics.
- [x] Scan every mounted stock MouseRect interaction-model instance and MouseFlags token. The A339X definitions and vendored stock ModelBehaviorDefs use only `IMDefault` and `IMDrag`; every literal MouseFlags token found is covered by the supported event vocabulary or its `LeftAll`/`RightAll`/`MiddleAll`/`Wheel` expansion. Remaining parameter placeholders expand through the same authored values, while unknown future models and events retain explicit fail-closed diagnostics.
...
- [x] Verify model-specific cursor, tooltip, timing, scale, and lock metadata on focused stock fixtures. Mounted A339X verification confirmed `Grab` plus Y scale `-450` and authored drag/temporary-lock flags on throttle MouseRect bindings, `Hand` plus 0.5/0.25 second button timing on EFB/MCDU controls, and directional tooltip action metadata on FCU baro and RMP tuning controls.
