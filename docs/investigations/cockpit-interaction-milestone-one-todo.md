...
- [~] Complete direct browser acceptance for hit testing and replacement paths. Interaction meshes, fallback hitboxes, passive gauges, LOD replacement, live target disappearance, and a real mounted blocker are verified. Agent Browser panned to the live skinned `KNOB_CHRONO_GPS` blocker and a physical click produced `lastHitKind=blocker`, `lastMissReason=blocked`, no active interaction, and no camera movement. Authored cover relationships remain unproven in the current IR and are the remaining geometry/arbitration gap.
