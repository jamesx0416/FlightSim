...
- [x] Define simulator agnostic canonical airframe and propulsion physics metadata.
- [x] Translate authoritative MSFS `flight_model.cfg` and `engines.cfg` metadata into that definition.
- [x] Add atmosphere state for altitude, temperature, pressure, density, speed of sound, Mach, and wind.
- [x] Add a fixed step rigid body air physics subsystem in the engine `physics` scheduler phase.
- [x] Use spanwise wing elements so local airflow, roll rate, control deflection, flaps, and spoilers affect forces at their physical locations.
  ...
  - [x] Drive asymmetric left/right wing flex from distributed element bending loads.
- [x] Apply engine thrust at each authored engine location so asymmetric thrust creates the correct moments.
- [x] Drive thrust from N1, Mach, static thrust, thrust scalar, and the package thrust table.
- [x] Keep starter, ignition, fuel availability, spool state, and generator availability coupled to existing canonical propulsion systems.
- [x] Expose physics state and controls through `window.__DevApi` for deterministic browser testing.
...
- [x] Add spatially and temporally varying turbulence and gust fields.
- [ ] Add icing, precipitation, and weather-driven wind fields.
- [x] Add dynamic aircraft mass, CG, inertia, and fuel burn from tank state.
- [x] Drive rendered wing flex from computed distributed loads through authored MSFS WingFlex nodes.
- [ ] Couple structural deflection back into local aerodynamic geometry for full aeroelastic feedback.
