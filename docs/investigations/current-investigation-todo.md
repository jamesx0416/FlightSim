...
- Findings:
  ...
  - HTML gauge capture now defaults to live refresh with generic instrument `Update()` scheduling aligned to the texture capture cadence; `?vcockpitGaugeUpdateMs=` or `?vcockpitGaugeUpdateHz=` can force periodic iframe updates, `?vcockpitGaugeUpdateMs=off` restores every-animation-frame updates, and `?vcockpitLiveGauges=off` switches to a bounded first-successful-frame pass that caches captured pixels and releases hidden iframes
  ...
  - gear, flap/slat, spoiler, and parking-brake XML-facing control variables are now dynamic runtime fallbacks instead of one-time stored defaults; Agent Browser verified the A330 route keeps generic `A:` control vars and common local aliases such as `L:A32NX_GEAR_CENTER_POSITION`, `L:A32NX_FLAPS_HANDLE_PERCENT`, `L:A32NX_SPOILERS_HANDLE_POSITION`, and `L:A32NX_PARK_BRAKE_LEVER_POS` synchronized through `GEAR_UP`/`GEAR_DOWN`, `FLAPS_INCR`/`FLAPS_DECR`, `SPOILERS_SET`, and `PARKING_BRAKE_SET`
  - VCockpit surface binding can now bind screen meshes by generic object/mesh names when lower-detail models reuse another display material; static A320 LOD1 verification maps `PFD_R` to `SCREEN_PFD_R` and `ND_R` to `SCREEN_MFD_R` without binding their glass overlay meshes, and live A320 LOD1 verification reached 24/24 loaded gauges, 18 captured gauges, no unresolved surface material warnings, active cockpit LOD1, and about 60 FPS
  - live A320 LOD1 MCDU verification loaded and captured `A32NX/MCDU/mcdu.html` without script/resource errors; DevApi clicks on `PUSH_MCDUL_MENU` and `PUSH_MCDUL_L1` dispatched the expected `A320_Neo_CDU_1_BTN_MENU` and `A320_Neo_CDU_1_BTN_L1` HTML events into the runtime
  - WASM bridge gauges now expose the resolved package `.wasm` URL in gauge bridge diagnostics by default without browser-side compile/import/export inspection; native MSFS WASM ABI execution remains explicitly unsupported
- Plan:
