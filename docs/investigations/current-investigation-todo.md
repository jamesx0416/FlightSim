...
- Findings:
  ...
  - synthetic WASM bridge hosts now expose the resolved package `.wasm` URL in gauge bridge diagnostics by default, and `?vcockpitWasmInspect=1` can opt into browser-side `WebAssembly.compile()` import/export inspection without adding that cost to normal cockpit startup
- Plan:
