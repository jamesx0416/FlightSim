...
- Findings:
  ...
  - VCockpit surface binding can now bind screen meshes by generic object/mesh names when lower-detail models reuse another display material; static A320 LOD1 verification maps `PFD_R` to `SCREEN_PFD_R` and `ND_R` to `SCREEN_MFD_R` without binding their glass overlay meshes
- Plan:
